    ...
    <div style="height: calc(100% - 64px)" class="p-5 bg-gray-50">
      <div class="w-full h-16 p-4 flex divide-x shadow-md rounded-md bg-white">
        ...
        <div class="w-1/2 h-full flex">
          <div class="flex-grow divide-x flex p-2 text-gray-400 text-sm leading-4">
            <div class="px-3 flex">
              <input
                class="w-11 h-8 -m-2 outline-none inline-block mr-1 border p-2 rounded-md"
                type="number"
                :value="countdown"
          ...
          <div class="w-14 h-full">
            <button
              class="w-full h-6 leading-3 text-blue-100 text-xs border p-1 border-blue-200 rounded-md mt-1"
      ...
      <div class="w-full overflow-y-scroll" style="height: calc(100% - 120px)">
        ...
        >
          <div
            class="w-1/2 pl-5 overflow-ellipsis whitespace-nowrap break-all overflow-x-hidden"
          >{{ timer.content }}</div>
          <div class="w-1/4 pl-5">{{ formatCountdown(timer.last) }}</div>
          <div class="w-1/4 pl-5">
            {{ formatCountdown(timer.remain) }}
            <span
              class="float-right text-blue-500 pr-3 opacity-0 cursor-pointer group-hover:opacity-100"
